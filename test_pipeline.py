...
class TestPipeline(unittest.TestCase):
    ...
    def test_blur_filter(self):
            # Создаем канал для изображения
            input_channel = Channel("image", str)
            output_channel = Channel("image", str)

            # Создаем узел для фильтра blur
            node = Node(
                name="BlurNode",
                inputs={"image": input_channel},
                outputs={"image": output_channel},
                func=blur
            )

            # Отправляем данные
            input_channel.send("image.png")
            ...
            # Проверяем результат
            result = output_channel.receive()
            self.assertEqual(result, "image.png [Blur applied]")
